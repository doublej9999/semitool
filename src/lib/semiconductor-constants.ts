...
export const HANDBOOK_FORMULAS: HandbookTopic[] = [
  ...
  {
    id: 'cu-electroplating-damascene',
    category: 'Metallization & Interconnect',
    title: 'Faraday’s Law of Electrolysis & CEAC Damascene Superfilling',
    formulaLatex: 'v_{dep} = \\frac{J \\cdot M_{Cu}}{n \\cdot F \\cdot \\rho_{Cu}} \\cdot \\eta_{eff},\\quad \\frac{d\\theta_{acc}}{dt} = k_a C_{acc} (1-\\theta_{acc}) + v \\cdot \\kappa \\cdot \\theta_{acc}',
    formulaPlain: 'v_dep = (J · M_Cu) / (n · F · ρ_Cu) · η_eff,   dθ/dt = ka·C_acc·(1-θ) + v·κ·θ',
    variables: [
      { symbol: 'v_dep', meaning: 'Copper electrodeposition rate', unit: 'nm/min or µm/min' },
      { symbol: 'J', meaning: 'Cathode current density', unit: 'mA/cm²' },
      { symbol: 'M_Cu', meaning: 'Copper atomic molar mass (63.546 g/mol)', unit: 'g/mol' },
      { symbol: 'n', meaning: 'Valence electron exchange (2 for Cu²⁺)', unit: '—' },
      { symbol: 'F', meaning: 'Faraday constant (96,485 C/mol)', unit: 'C/mol' },
      { symbol: 'ρ_Cu', meaning: 'Copper bulk density (8.96 g/cm³)', unit: 'g/cm³' },
      { symbol: 'η_eff', meaning: 'Faradaic current efficiency', unit: '%' },
      { symbol: 'θ_acc', meaning: 'Accelerator additive surface coverage', unit: 'fraction (0–1)' },
    ],
    explanation:
      'Models dual damascene electrochemical deposition (ECD). Faraday’s law calculates planar plating rate, while Curvature Enhanced Accelerator Coverage (CEAC) explains bottom-up superfilling of high aspect ratio trenches and vias without void defects.',
    fabRelevance: 'Back-End-of-Line (BEOL) copper interconnect fabrication, TSV (Through-Silicon Via) filling, and seed layer terminal effect compensation.',
    calculatorPath: '/tools/cu-plating-calculator',
  },
  {
    id: 'wet-bench-kinetics',
    category: 'Wet Chemical & Cleans',
    title: 'Wet Chemical Dissolution & Bath Lifetime Decay',
    formulaLatex: 'ER(t) = ER_0 \\cdot \\exp(-k_{decay} \\cdot t) \\cdot \\left(1 - \\frac{M_{dissolved}}{M_{capacity}}\\right)',
    formulaPlain: 'ER(t) = ER_0 · exp(-k_decay · t) · (1 - M_dissolved / M_capacity)',
    variables: [
      { symbol: 'ER(t)', meaning: 'Instantaneous etch or strip rate', unit: 'nm/min or Å/min' },
      { symbol: 'ER_0', meaning: 'Fresh bath initial baseline etch rate', unit: 'nm/min' },
      { symbol: 'k_decay', meaning: 'Thermal breakdown / oxidant degradation coefficient', unit: '1/hr' },
      { symbol: 't', meaning: 'Bath cumulative operational age', unit: 'hours' },
      { symbol: 'M_dissolved', meaning: 'Cumulative dissolved wafer material mass', unit: 'g or ppm' },
      { symbol: 'M_capacity', meaning: 'Bath chemical saturation exhaustion limit', unit: 'g or ppm' },
    ],
    explanation:
      'Tracks wet clean chemistry performance across RCA SC-1 (NH4OH/H2O2/H2O), SC-2 (HCl/H2O2/H2O), Piranha/SPM (H2SO4/H2O2), and BOE (NH4F/HF). Accounts for oxidant decomposition, chemical loading saturation, and spike replenishment spiking.',
    fabRelevance: 'Optimizes chemical bench bath lifetime, wafer cycle times, spike replenishment schedules, and prevents particle re-deposition.',
    calculatorPath: '/tools/wet-bench-calculator',
  },
  {
    id: 'stoney-wafer-stress',
    category: 'Thin Film Stress & Mechanical',
    title: 'Modified Stoney Equation for Thin Film Stress & Wafer Bow',
    formulaLatex: '\\sigma_f = \\frac{E_s \\cdot t_s^2}{6(1-\\nu_s) \\cdot t_f} \\left(\\frac{1}{R_{post}} - \\frac{1}{R_{pre}}\\right) \\approx \\frac{4 E_s \\cdot t_s^2 \\cdot \\Delta \\text{Bow}}{3(1-\\nu_s) \\cdot t_f \\cdot D_w^2}',
    formulaPlain: 'σ_f = [E_s · t_s² / (6(1-ν_s) · t_f)] · (1/R_post - 1/R_pre) ≈ 4·E_s·t_s²·ΔBow / [3(1-ν_s)·t_f·D_w²]',
    variables: [
      { symbol: 'σ_f', meaning: 'Internal thin film stress (positive = tensile, negative = compressive)', unit: 'MPa' },
      { symbol: 'E_s / (1-ν_s)', meaning: 'Substrate biaxial elastic modulus (180.5 GPa for Si (100))', unit: 'GPa' },
      { symbol: 't_s', meaning: 'Substrate wafer thickness', unit: 'µm' },
      { symbol: 't_f', meaning: 'Deposited thin film thickness', unit: 'nm or µm' },
      { symbol: 'R_post, R_pre', meaning: 'Substrate radius of curvature after and before deposition', unit: 'm' },
      { symbol: 'ΔBow', meaning: 'Wafer center-to-edge bow shift', unit: 'µm' },
      { symbol: 'D_w', meaning: 'Wafer diameter (e.g. 200 mm or 300 mm)', unit: 'mm' },
    ],
    explanation:
      'Derives average internal mechanical film stress from macroscopic substrate curvature or wafer bow distortion. Separates intrinsic film stress from thermal mismatch stress caused by differential thermal expansion (CTE mismatch) during cooling from deposition temperature.',
    fabRelevance: 'Essential for wafer handling safety in automated robots (preventing vacuum chuck slippage), lithography overlay focal distortion, and wafer cracking/delamination control.',
    calculatorPath: '/tools/wafer-warp-stress-calculator',
  },
];
